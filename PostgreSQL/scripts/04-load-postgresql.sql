...
DO $$
DECLARE
  folder TEXT := '/SnomedCT'; -- Change the root directory based on your file location
  type TEXT := 'Full'; -- Change between Full, Delta, Snapshot.
  release TEXT := 'INT_20220930'; -- Change between each release
  suffix TEXT := ''; -- Suffix of the database table. _f stands for full, _d stands for delta, _s stands for snapshot
BEGIN
...
--   EXECUTE 'COPY extendedmaprefset' || suffix || '(id, effectivetime, active, moduleid, refsetid, referencedcomponentid, mapGroup, mapPriority, mapRule, mapAdvice, mapTarget, correlationId) FROM '''
--         || folder || '/' || type || '/Refset/Map/der2_iissscRefset_ExtendedMap' || type || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, DELIMITER ''	'')';
